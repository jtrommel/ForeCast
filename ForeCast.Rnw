...
for (i in (2:nrow(serie.des))) {
  if (i==2) {
    ...
    serie.des$level[i] <- alpha*serie.des$y[i] + (1-alpha)*(serie.des$level[i-1] + serie.des$slope[i-1])
    serie.des$slope[i] <- beta*(serie.des$level[i] - serie.des$level[i-1]) + (1-beta)*serie.des$slope[i-1]
  }
  serie.des$yhat[i] <- serie.des$level[i] + serie.des$slope[i]
}
